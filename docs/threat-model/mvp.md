...
| Adversarial light or material definitions create unbounded per-frame GPU work, invalid directions, non-finite positions, singular direct-light math, unbounded emission, or ambiguous alpha coverage | High | Independent four-definition directional/point caps, stable preparation, fixed uniform, finite/unit-bounded scene and imported values, unit-bounded surface-only emission with final RGB clamp, strict finite non-negative OPAQUE/MASK coverage with pre-output discard and BLEND exclusion, finite selected-camera conversion, roughness and BRDF denominator floors, degenerate active-direction and out-of-range active-position rejection, exact-zero and derived-distance-overflow handling, and pre-submit tests | Low |
