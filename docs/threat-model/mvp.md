...
| Malformed, substituted, or adversarial GLB/PNG allocates excessively or reaches GPU state | High | Service-owned exact-hash admission, source/decoded/count limits, strict geometry/image subset, exact 64-byte expanded-vertex reservation, finite non-zero same-count normal and tangent with exact handedness, full-source finite same-count primary-coordinate validation, strict finite/clamped or normalized primary-color validation before wider-set proxy classification, at most sixteen primitive attribute semantics with aliased color accessors validated once, two checked fixed generated-tangent work guards before exact-pinned corrected MikkTSpace and complete finite/non-zero/handedness output validation, strict finite texture-transform decoding and complete expanded affine-result validation with transformed normal-coordinate generation, unit-bounded numeric and three-channel emissive material, strict selected/unused boolean `doubleSided`, strict extension declaration and exact-empty unlit marker preflight without retained maps, strict zero-to-four selected/unused core sampler records and texture indices before proxy classification, finite normal-scale validation, zero-to-four typed texture roles with unique-image CPU accounting, PNG dimension/pixel/working/decoded bounds, per-asset and aggregate proxy/decoded byte enforcement, atomic content-hash-and-role GPU texture count/byte reservation, exact-pinned vendored decoders/generator, explicit one-item processing, empty recovery residency, truncation corpus, unsafe proxy exclusions | Medium |
...
| Adversarial light or material definitions create unbounded per-frame GPU work, invalid directions, non-finite positions, singular direct-light math, unbounded emission, ambiguous alpha coverage, or unbounded pipeline/sampler variants | High | Independent four-definition directional/point caps, stable preparation, one fixed 624-byte uniform with four prevalidated affine transforms, finite/unit-bounded scene and imported values, unit-bounded surface-only emission with final RGB clamp, strict finite non-negative OPAQUE/MASK coverage with pre-output discard and BLEND exclusion, exactly two initialization-owned shared-layout face pipelines and one fixed 36-entry sampler table selected per draw without keyed caches, explicit four-sampler/nine-binding adapter preflight, finite selected-camera conversion, roughness and BRDF denominator floors, degenerate active-direction and out-of-range active-position rejection, exact-zero and derived-distance-overflow handling, and pre-submit tests | Low |
