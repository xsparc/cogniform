...
| GLB tangent range/count/handedness is invalid, a normal-textured primitive omits its source basis, or texture roles are inconsistent | Reject malformed source data without proxy; only valid-but-unsupported basis/encoding cases may use the explicit proxy policy; never partially reserve GPU roles | CF055 tangent, role, and atomic-reservation tests |
...
| Draw, mesh-upload, or role-texture residency pressure | Reject before GPU preparation/allocation; atomically reserve only missing content-hash-and-role textures | renderer scene and CF029/CF055/CF056 asset tests |
...
| Built-in cuboid topology or orientation regresses | Initialization-only tests require six faces, two non-degenerate outward triangles per face, exact axis normals, zero primary coordinates, disabled fallback tangents, 36 vertices, and the fixed 1,728-byte payload | CF025/CF028/CF055 renderer unit and controlled adapter tests |
| Built-in sphere generation or selection regresses | Initialization-only topology tests require the exact finite 672-vertex, 32,256-byte payload, outward winding, unit radial normals, zero primary coordinates, disabled fallback tangents, direct/fallback selection, and resident-asset precedence; frames never tessellate or substitute another shape | CF022/CF028/CF055 renderer unit and scene tests |
