...
| Offline asset inspection sees invalid hash syntax, a non-file, over-limit/growing source, or hash substitution | Reject before success output; keep stdout empty and diagnostics path/payload-redacted; perform no decode, service, network, upload, or GPU work | CF036 CLI inspection and existing storage tests |
