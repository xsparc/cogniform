...
| A `KHR_texture_transform` payload is malformed, undeclared, non-finite, selects a nonzero coordinate set, has a wider property, or produces a non-finite expanded coordinate | Reject malformed/undeclared/non-finite input and non-finite affine results without proxy; classify only valid nonzero selectors or wider properties as unsupported after malformed peers; never treat them as identity or expose a partial asset | CF066 declaration, precedence, affine-overflow, generated-tangent, and four-role renderer tests |
