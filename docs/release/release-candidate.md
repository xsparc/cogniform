...
| Asset safety | Pass for documented GLB subset | Exact hash, strict framing/ranges/counts, finite non-zero same-count normals/tangents with exact handedness, finite same-count primary coordinates with full-source validation, bounded numeric materials and normal scale, bounded static embedded RGB/RGBA PNG decode, shared/distinct role accounting, exact decoded/GPU bytes, truncation corpus, typed unsupported/proxy policy |
| Service asset resolution | Pass on validated profile | Explicit one-item import/upload renders an exact stable ID and optional base-color/normal role textures; recovery retains logical references and exact-hash rehydration resumes rendering without replay mutation |
...
Checked items below were reproduced through CF055 and are recorded in the
validation baseline; the package/source identity matrix originated on CF051
and the executable MCP evidence was revalidated on CF054. The CF055 controlled
GPU path was reproduced separately on the same validated adapter. Pull-request quality
...
- normal output is quantized and retains documented geometric smoothing while
  direct lighting may use one source-tangent normal map; generated tangents,
...
- built-in rendering supports outward-wound centered unit cuboids, fixed
  ...
  embedded PNG base-color and normal roles can sample imported primary coordinates;
...
- lighting supports independently bounded sets of at most four stable-ordered
  ...
  conversion, additional image formats/samplers/material texture roles, and lighting
