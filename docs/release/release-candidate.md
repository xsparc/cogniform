...
- normal output is quantized and retains documented geometric smoothing while
  ...
  and one linear packed metallic-roughness map; retained primary coordinates
  can use bounded per-role ratified affine transforms, but generated coordinates,
...
- built-in rendering supports outward-wound centered unit cuboids, fixed
  ...
  filtering, independent repeat/mirrored/clamp wrapping, and independent
  offset/rotation/scale transforms;
