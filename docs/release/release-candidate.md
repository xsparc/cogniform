...
| Headless render | Pass on validated profile | No surface/window; outward-wound Vulkan cuboid, centered plane, fixed sphere, bounded directional/point direct metallic-roughness response including imported numeric GLB materials, ratified base-color-only imported unlit response, interpolated primary vertex color, core emissive factors, deterministic OPAQUE/MASK coverage, fixed single/double-sided imported face behavior, sampled embedded PNG base-color, packed metallic-roughness, explicit or bounded generated-tangent normal, and sRGB emissive roles with independent bounded core filtering/wrapping, scene overrides, geometric-normal observation preservation, readback pressure, renderer-drop retirement, and canonical scenario evidence |
| Machine outputs | Pass on validated profile | Exact no-active-light and imported-unlit sampled color plus tolerant scene/imported/overridden direct-material color and depth, exact stable entity ID, structured visibility including culled imported back faces, and quantized world-space normals from outward built-ins, source-wound assets, sphere radial directions, approved imported directions, or face-oriented explicit double-sided back faces |
...
Checked items below were reproduced through CF065 and are recorded in the
...
CF065 controlled GPU paths were reproduced separately on the same validated adapter. Pull-request quality
...
- normal output is quantized and retains documented geometric smoothing while
  direct lighting may use one explicit or bounded generated-tangent normal map
  and one linear packed metallic-roughness map; generated coordinates,
...
- built-in rendering supports outward-wound centered unit cuboids, fixed
  ...
  embedded PNG base-color, metallic-roughness, normal, and emissive roles can
  sample imported primary coordinates with bounded core nearest/linear
  filtering and independent repeat/mirrored/clamp wrapping;
...
- lighting supports independently bounded sets of at most four stable-ordered
  ...
  occlusion textures, additional image formats, non-core sampler controls,
  generated/stored mipmaps, additional material texture roles, and lighting
